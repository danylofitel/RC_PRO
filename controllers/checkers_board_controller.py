...
FILLER_MAP = {
    ...
    1: "checker_white",
    2: "checker_black",
    3: "checker_king_white",
    4: "checker_king_black",
}
...
class CheckersBoardController(CommonBoardController):
                ...
                if not bot_move is None:
                    if self.controller.should_show_bot_play_on_board:
                        if self.controller.model.last_move_type == MOVE_TYPES["simple"]:
                            ...
                            self.controller.model.last_move_type == MOVE_TYPES["attack"]
                        ):
                            self.controller.attack_move(
                                bot_move[0][0],
                                bot_move[0][1],
                                bot_move[1][0],
                                bot_move[1][1],
                    ...
                    if (
                        self.controller.should_log_the_game
                        or self.controller.should_log_the_game_on_board
                    ):
                        txt_to_write = (
                            "Move of player "
                            + str(current_player)
                    ...
                    if (
                        self.controller.model.current_player
                        == self.controller.bot.player
    ...
    def on_cell_click(self, event, x, y):
        if (
            ...
            or self.game_mode == GAME_MODES["playerVSPro"]
        ):
            if self.state == STATES["None"] and self.model.is_able_to_move(x, y):
                ...
                if not self.last_selected is None:
                    ...
                    available_moves = self.model.get_available_moves(
                        self.last_selected[0], self.last_selected[1]
                    )

                    if (
                        self.last_selected[0] == x
                        and self.last_selected[1] == y
                        and self.model.is_free_to_deselect()
                    ):
                        should_bot_move_after = False
                        should_move_to_none_state = True

                    elif (x, y) in self.model.get_available_moves(
                        self.last_selected[0], self.last_selected[1]
                    ):
                        move_success = self.model.move(
                            self.last_selected[0], self.last_selected[1], x, y
                        )
                        if move_success:
                                ...
                                if self.model.should_continue_attack():
                                    for a in available_moves:
                                        self.change_cell_color(
                                            a[0], a[1], CHECKERS_BACKGROUND_COLOR
                                        )
                                    self.change_cell_color(
                    ...
                    if (
                        self.game_mode == GAME_MODES["playerVSPro"]
                        and should_move_to_none_state
                        and should_bot_move_after
                    ):
                    ...
                    if self.model.winner == 0:
                        ...
                        if self.should_log_the_game_on_board:
                            self.write_to_console(
                                "The winner is player " + str(self.model.winner)
                            )
                    self.the_end = True
                        ...
                        if self.controller.model.winner == 0:
                            ...
                            if self.controller.should_log_the_game:
                                print(
                                    ...
                                    + str(self.controller.model.winner)
                                )
                            if self.controller.should_log_the_game_on_board:
                                self.controller.write_to_console(
                                    "The winner is player "
                                    + str(self.controller.model.winner)
                                )
